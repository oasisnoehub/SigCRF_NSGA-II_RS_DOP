...
def fit_score(population,model,X_train,X_train_val,X_test,y_train,y_train_val,y_test):
    ...
    for chromosome in population:
        ...
        predictions_proba = cal_clf.predict_proba(X_test.iloc[:, chromosome])
        # 评价分数
...
# best_chromo ： 每一代最优的染色体
# best_score ： 最优染色体对应的分数
def generations_v2(X_train,X_train_val,y_train,y_train_val,X_test,y_test,model,pop_size,num_features,drop_rate = 0.3,mutation_rate=0.3,num_generation=10):
    ...
    for i in range(num_generation):
        ...
        plt.ylabel("Error Distance(1 - Average f1 score)",fontsize=15)
        plt.xticks(fontsize=15)
        plt.yticks(fontsize=15)
        # plt.title("Two Targets Optimization")
        plt.savefig(f'../gaImages/generation_{i+1}.png',dpi=600)
        # plt.show()
